...
inline Bitboard get_outposts(const Position& position)
{
    ...
    if (popcount(pawn_on_fileB))
        sq_fileB =
            Square(side == WHITE ? msb(pawn_on_fileB) : lsb(pawn_on_fileB));
    else
    ...
    if (popcount(pawn_on_fileG))
        sq_fileG =
            Square(side == WHITE ? msb(pawn_on_fileG) : lsb(pawn_on_fileG));
    else
    ...
    for (int i = 1; i < 7; ++i)
    {
        ...
        if (popcount(pawn_on_file1_bb))
            sq_file1 = Square(side == WHITE ? msb(pawn_on_file1_bb)
                                            : lsb(pawn_on_file1_bb));
        else
            sq_file1 = make_square(side == WHITE ? RANK_1 : RANK_8, file1);

        if (popcount(pawn_on_file2_bb))
            sq_file2 = Square(side == WHITE ? msb(pawn_on_file2_bb)
                                            : lsb(pawn_on_file2_bb));
        else
            sq_file2 = make_square(side == WHITE ? RANK_1 : RANK_8, file2);

        outpost_bb |= (squares_left_behind_bb<!side>(sq_file1) &
                       squares_left_behind_bb<!side>(sq_file2));
    }
    ...
    Bitboard attacked_squares =
        shift<forward_left>(pawns) | shift<forward_right>(pawns);
