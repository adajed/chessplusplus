...
Bitboard get_blockers_from_index(int index, Bitboard mask)
{
    ...
    for (int i = 0; i < num_bits; ++i)
    {
        ...
        if (index & (1 << i)) blockers |= square_bb(position);
    }
    return blockers;
}
...
void init_rays()
{
    Direction directions[] = {NORTHWEST, NORTH, NORTHEAST, EAST,
...
void init_bishop_magics()
{
    for (Square sq = SQ_A1; sq <= SQ_H8; ++sq)
    {
        for (int i = 0; i < 4096; ++i) BISHOP_TABLE[sq][i] = all_squares_bb;
...
void init_rook_magics()
{
    for (Square sq = SQ_A1; sq <= SQ_H8; ++sq)
    {
        for (int i = 0; i < 4096; ++i) ROOK_TABLE[sq][i] = all_squares_bb;
...
void init_lines_bitboards()
{
    ...
    for (Square from = SQ_A1; from <= SQ_H8; ++from)
        for (Square to = SQ_A1; to <= SQ_H8; ++to) LINES[from][to] = 0ULL;
        ...
        for (Square to = SQ_A1; to <= SQ_H8; ++to)
        {
            ...
            else if ((r_from + f_from) == (r_to + f_to))
            {
                ...
                for (int f = 0; f < 8; ++f)
                {
                    int r = c - f;
                    if (0 <= r && r < 8)
                        FULL_LINES[from][to] |= square_bb(make_square(
