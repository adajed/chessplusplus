...
void Search::go()
{
    ...
    while (!stop_search)
    {
        ...
        _ply_counter = 0;
        Score result = root_search(_position, _current_depth, -INFINITY_SCORE, INFINITY_SCORE);
...
#ifdef DEBUG
        std::cout << "[" << depth << "] Searching root move: " << position.move_to_string(move) << std::endl;
#endif
...
#ifdef DEBUG
        std::cout << "[" << depth << "] Done root searching move: "
