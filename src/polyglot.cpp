...
const HashKey POLYGLOT_CASTLING_WHITE_SHORT = U64(0x31D71DCE64B2C310);
const HashKey POLYGLOT_CASTLING_WHITE_LONG  = U64(0xF165B587DF898190);
const HashKey POLYGLOT_CASTLING_BLACK_SHORT = U64(0xA57E6339DD2CF3A0);
const HashKey POLYGLOT_CASTLING_BLACK_LONG  = U64(0x1EF6E6DBB1961EC9);
...
const HashKey POLYGLOT_TURN = U64(0xF8D626AAAF278509);
    ...
    : _hashmap()
{
    ...
    while (stream)
    {
        ...
        if (promotion != 0)
            promotion = KNIGHT + promotion_code - 1;
...
HashKey PolyglotBook::hash(const Position& position) const
{
    HashKey key = 0ULL;
    ...
    if (position.castling_rights() & W_OO)
        key ^= POLYGLOT_CASTLING_WHITE_SHORT;
    if (position.castling_rights() & W_OOO)
        key ^= POLYGLOT_CASTLING_WHITE_LONG;
    if (position.castling_rights() & B_OO)
        key ^= POLYGLOT_CASTLING_BLACK_SHORT;
    if (position.castling_rights() & B_OOO)
        key ^= POLYGLOT_CASTLING_BLACK_LONG;
...
Move PolyglotBook::sample_move(HashKey key, const Position& position) const
{
    ...
    std::vector<WeightedMove> moves = _hashmap[key];
...
{
    if (from(move) == SQ_E1 && to(move) == SQ_H1 && position.piece_at(from(move)) == W_KING)
        return create_castling(KING_CASTLING);
    if (from(move) == SQ_E1 && to(move) == SQ_A1 && position.piece_at(from(move)) == W_KING)
        return create_castling(QUEEN_CASTLING);
    if (from(move) == SQ_E8 && to(move) == SQ_H8 && position.piece_at(from(move)) == B_KING)
        return create_castling(KING_CASTLING);
    if (from(move) == SQ_E8 && to(move) == SQ_A8 && position.piece_at(from(move)) == B_KING)
        return create_castling(QUEEN_CASTLING);
    return move;
}
